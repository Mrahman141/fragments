...
async function deleteFragment(ownerId, id) {
  // S3 params for deleting the data object
  const s3Params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    Key: `${ownerId}/${id}`,
  };

  // DynamoDB params for deleting the metadata item
  const ddbParams = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    Key: {
      ownerId: ownerId,
  ...
  // Commands for deleting from S3 and DynamoDB
  ...
  try {
    ...
    logger.error({ err, Bucket, Key }, 'Error deleting fragment data from S3 or metadata from DynamoDB');
